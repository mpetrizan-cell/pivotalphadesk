...
NQ_SYMBOL        = "NQZ26"                   # futuro activo Dic 2026
...
# de Windows queda desactualizado (mismo fix aplicado en ts_gaia_etf.py, 12-ago-2026).
...
def read_stream_ndx(token, expiration, spot):
    ...
    try:
        ...
        # ts_gaia_vix.py (21-ago y 26-ago), portado acá tras confirmar en vivo un
        # cuelgue real de 3h34m en NDX el 28-ago (proceso vivo, mudo, sin
        # excepción — tuvo que matarse a mano). Causa: si TradeStation cierra la
        # conexión, readline() deja de bloquear y devuelve "" o basura no-JSON en
        # loop infinito; sin contador, ni lines_read ni heartbeats avanzan nunca
        # y el while gira a velocidad de CPU para siempre. Se cuenta como
        # improductiva CUALQUIER lectura que no sea un heartbeat o un dato real
        # de opción — vacía o basura no parseable, da igual — y se corta el
        # ciclo tras 50 seguidas.
        ...
        while lines_read < max_contracts and heartbeats < max_heartbeat:
            ...
            if not raw:
                ...
                if empty_reads > MAX_EMPTY_READS:
                    log.warning(
                        f"Stream NDX sin datos tras {empty_reads} lecturas vacías "
            ...
            try:
                ...
                if empty_reads > MAX_EMPTY_READS:
                    log.warning(
                        f"Stream NDX sin datos utilizables tras {empty_reads} lecturas "
...
def save_ndx_json(layers_data, spot_ndx, spot_nq, basis, confluence,
    ...
    try:
        levels_0dte = layers_data.get("0dte", {}).get("levels", {})

        # levels_nq — niveles convertidos a NQ coordinates (igual que levels_es en SPX)
        levels_nq = {k: round(v + basis, 2) for k, v in levels_0dte.items()} if basis != 0 else levels_0dte.copy()

        # strikes flat — 0DTE con scores (igual que SPX top-level strikes)
        # layers_output usa key "strikes" (no "strikes_data")
        strikes_0dte = layers_data.get("0dte", {}).get("strikes", [])
        if not strikes_0dte:
            strikes_0dte = layers_data.get("0dte", {}).get("strikes_data", [])

        # call_delta_oi / put_delta_oi — compatibilidad con terminal v10
        # NDX usa call_gex/put_gex en lugar de call_delta_oi/put_delta_oi
        # Agregamos alias para que el terminal v10 funcione sin cambios
        strikes_compat = []
        for s in strikes_0dte:
            s_copy = dict(s)
            # Alias para compatibilidad con terminal v10
            s_copy["call_delta_oi"] = abs(s.get("call_gex", 0)) * 100
            s_copy["put_delta_oi"]  = abs(s.get("put_gex",  0)) * 100
            strikes_compat.append(s_copy)

        output = {
            "timestamp":     datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"),
            "expiration":    layers_data.get("0dte", {}).get("expiration", ""),
            "spot_ndx":      spot_ndx,
            ...
            # HIRO accumulated (CVD equivalent)
            ...
            # Levels in NDX coordinates
            "levels":        levels_0dte,
            # Levels in NQ coordinates (basis applied)
            "levels_nq":     levels_nq,
            "confluence":    confluence,
            # Strikes flat — top level (compatible con terminal v10)
            "strikes":       strikes_compat,
            "layers": {
        ...
        try:
            ...
            if spot_ndx > 0:
                ...
                save_ndx_json(layers_output, spot_ndx, spot_nq, basis,
                              ...
                              hiro_call_accum, hiro_put_accum)
                # Build payload completo igual que save_ndx_json
                strikes_0dte = layers_output.get("0dte", {}).get("strikes", [])
                strikes_compat = []
                for s in strikes_0dte:
                    s_copy = dict(s)
                    s_copy["call_delta_oi"] = abs(s.get("call_gex", 0)) * 100
                ...
                levels_0dte = layers_output.get("0dte", {}).get("levels", {})
                levels_nq = {k: round(v + basis, 2) for k, v in levels_0dte.items()} if basis != 0 else levels_0dte.copy()
